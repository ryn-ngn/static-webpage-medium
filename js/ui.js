...
let button = document.body.querySelector('#search-btn');
  if (isLoading){
    // change icon to loading
    button.children[0].className = 'fas fa-hourglass-half';
    // change text to "Loading..."
    button.querySelector('#button-text').textContent = 'Loading...';
    // disable button (can't be clicked)
    button.disabled = true;
  }

  if (!isLoading) {
    // change icon to magnifying glass
    button.children[0].className = 'fas fa-search';
    // change text to "Search"
    button.querySelector('#button-text').textContent = 'Search';
    // enable button
